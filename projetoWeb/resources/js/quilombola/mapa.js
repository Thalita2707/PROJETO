...
// Inicializa o mapa
const map = L.map('map').setView([-21.141, -55.825], 14);
...
comunidades.forEach((comunidade) => {
    const marker = L.marker([comunidade.lat, comunidade.lng]).addTo(map);
    marker.bindPopup(`<b>${comunidade.name}</b><br>${comunidade.desc}`);

    const bounds = [
        [comunidade.lat - 0.0009, comunidade.lng - 0.00097],
        [comunidade.lat + 0.0009, comunidade.lng + 0.00097]
    ];
